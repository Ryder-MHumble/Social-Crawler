...
def _clean_joined_values(values: Iterable[Any]) -> str:
    seen: set[str] = set()
    result: list[str] = []
    for raw in values:
...
async def update_xhs_note(note_item: Dict):
    ...
    video_url = _clean_joined_values(get_video_url_arr(note_item))
    note_id = _clean_text(note_item.get("note_id"))
    xsec_token = _clean_text(note_item.get("xsec_token"))
    xsec_source = _clean_text(note_item.get("xsec_source")) or "pc_search"
    title = _clean_text(note_item.get("title") or note_item.get("desc", ""), max_length=255)
    desc = _clean_text(note_item.get("desc", ""))
    ...
    local_db_item = {
        ...
        "nickname": _clean_text(user_info.get("nickname")),  # User nickname
        "avatar": _clean_text(user_info.get("avatar")),  # User avatar
        ...
        "ip_location": _clean_text(note_item.get("ip_location", "")),  # IP location
        "image_list": _clean_joined_values(img.get('url', '') for img in image_list),  # Image URLs
        "tag_list": _clean_joined_values(
        ...
        "note_url": f"https://www.xiaohongshu.com/explore/{note_id}?xsec_token={xsec_token}&xsec_source={xsec_source}",  # Note URL
        "source_keyword": _clean_text(source_keyword_var.get()),  # Search keyword
        "xsec_token": xsec_token,  # xsec_token
    }
...
async def update_xhs_note_comment(note_id: str, comment_item: Dict):
    ...
    local_db_item = {
        ...
        "ip_location": _clean_text(comment_item.get("ip_location")),  # IP location
        "note_id": _clean_text(note_id),  # Note ID
        "content": _clean_text(comment_item.get("content")),  # Comment content
        "user_id": user_info.get("user_id"),  # User ID
        "nickname": _clean_text(user_info.get("nickname")),  # User nickname
        "avatar": _clean_text(user_info.get("image")),  # User avatar
        "sub_comment_count": comment_item.get("sub_comment_count", 0),  # Sub-comment count
        "pictures": _clean_joined_values(comment_pictures),  # Comment pictures
...
async def save_creator(user_id: str, creator: Dict):
    ...
    local_db_item = {
        'user_id': _clean_text(user_id),  # User ID
        'nickname': _clean_text(user_info.get('nickname')),  # Nickname
        'gender': get_gender(user_info.get('gender')),  # Gender
        'avatar': _clean_text(user_info.get('images')),  # Avatar
        'desc': _clean_text(user_info.get('desc')),  # Personal description
        'ip_location': _clean_text(user_info.get('ipLocation')),  # IP location
