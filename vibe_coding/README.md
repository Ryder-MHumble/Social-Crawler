...
这是一个单独的采集模块，用来抓取 AI 编程、零代码工具、独立开发一类内容。重点不是“全量收集”，而是尽快筛出值得看的样本。
...
### 核心功能
- **关键词分层评分**：用 Tier A/B/C 三档关键词做初筛
- **黑名单过滤**：排除明显无关的内容，例如"鼠标 cursor"、"bolt 螺栓"
- **多层去重**：
  - 内容 ID 去重（跨会话持久化）
  - 标题指纹去重（捕获跨平台转发）
- **互动量筛选**：只保留有基本讨论度的内容（点赞+评论 ≥ 5）
- **Top 评论收集**：自动保存每条内容的前 20 条高赞评论
- **趋势分类**：把结果归到 8 个常见主题里，方便后续分析
